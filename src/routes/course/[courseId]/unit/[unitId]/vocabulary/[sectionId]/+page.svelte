<script lang="ts">
	...
	import VocabularySection from '$components/section/VocabularySection.svelte';

	let { data } = $props<{ data: PageData }>();
	let { course, unit, section, unitSections = [], vocabularyItems = [] } = data;

	// Guard against undefined data
	if (!course) course = { id: '', title: '' } as any;
	if (!unit) unit = { id: '' } as any;
	if (!section) section = { id: '' } as any;
</script>
...
<div class="section-page">
	...
	{#if section && unit && unitSections.length > 0}
		...
		<VocabularySection {section} {vocabularyItems} />
	{:else}
